...
echo "metadata files count:"$(find assets -type f -name '*.json' | wc -l)
echo "images files count:"$(find assets -type f -name '*.png' | wc -l)
echo "total number of asset files:"$(find assets -type f  | wc -l)
echo ""
...
echo $(find assets -type f -name '*.json' | xargs jq -r '.name' | sort | cat) # was less
echo ""
